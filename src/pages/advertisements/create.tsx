...
import { useCreateAdvertisement } from '@/hooks/mutations/useAdvertisementMutations';
...
export default function CreateAdvertisement() {
  ...
  // State for phone numbers and media IDs
  ...
  const updatePhoneNumber = (index: number, value: string) => {
    ...
    setPhoneNumbers(newPhoneNumbers);
    // Live validate phone format: optional +, digits only, max 13 digits
    const regex = /^\+?[0-9]{1,13}$/;
    const newErrors = [...phoneErrors];
    if (value.trim() === '') {
      newErrors[index] = '';
    } else if (!regex.test(value)) {
      newErrors[index] = t('validation.phoneNumbers.invalid');
    } else {
      newErrors[index] = '';
    }
    setPhoneErrors(newErrors);

    // Reflect valid numbers into form state
    form.setValue('phone_numbers', newPhoneNumbers.filter(num => num.trim() !== ''));

    // If any invalid exists, set form error; otherwise clear
  ...
  const onSubmit = (data: any) => {
    ...
    const createData: CreateAdvertisementData = {
      ...
      phone_numbers: data.phone_numbers.filter((num: string) => num.trim() !== ''),
      email: data.email || '',
      status: data.status || 'draft',
      media_ids: mediaIds,
    };

    createAdvertisementMutation.mutate(createData, {
      onSuccess: () => {
        showSuccess(
          t('createAdvertisement.successMessage'),
          t('createAdvertisement.successTitle')
        );
        // Navigate to the advertisements list page
        setTimeout(() => {
      ...
      onError: (error: any) => {
        ...
        const errorMessage = error?.response?.data?.message || error?.message || t('createAdvertisement.errorMessage');
        showError(errorMessage, t('createAdvertisement.errorTitle'));
      },
    });
  };
  ...
  const handleRegionChange = (value: string | undefined) => {
    ...
    form.setValue('township_id', 0); // Reset township when region changes
  };

  return (
    <>
      <SEOHead seo={seo} path="/advertisements/create" />
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Card className="shadow-lg">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField 
                      name="title_en" 
                      label={t('createAdvertisement.titleEn')} 
                      error={errors.title_en} 
                      required
                    >
                      <Input
                        {...form.register('title_en')}
                        placeholder={t('createAdvertisement.titleEnPlaceholder')}
                      />
                    </FormField>

                    <FormField 
                      name="title_mm" 
                      label={t('createAdvertisement.titleMm')} 
                      error={errors.title_mm} 
                      required
                    >
                      <Input
                        {...form.register('title_mm')}
                        placeholder={t('createAdvertisement.titleMmPlaceholder')}
                      />
                    </FormField>
                  </div>
                  ...
                  <FormField 
                    name="description" 
                    label={t('createAdvertisement.descriptionLabel')} 
              ...
              <Card className="shadow-lg">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField 
                      name="region_id" 
                      label={t('createAdvertisement.region')} 
                      error={errors.region_id} 
                      required
                    >
                             <Select
                               value={form.watch('region_id') && form.watch('region_id') > 0 ? form.watch('region_id').toString() : undefined}
                    ...
                    <FormField 
                      name="township_id" 
                      label={t('createAdvertisement.township')} 
                      error={errors.township_id} 
                      required
                    >
                             <Select
                               value={form.watch('township_id') && form.watch('township_id') > 0 ? form.watch('township_id').toString() : undefined}
                               onValueChange={(value) => {
                                 const numeric = Number(value);
                  ...
                  <FormField 
                    name="address" 
                    label={t('createAdvertisement.address')} 
                    error={errors.address} 
                    required
                  >
                    <Input
                      {...form.register('address')}
                      placeholder={t('createAdvertisement.addressPlaceholder')}
                    />
                  </FormField>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      {t('createAdvertisement.phoneNumbers')} <span className="text-red-500">*</span>
                    </label>
                    {phoneNumbers.map((phone, index) => (
                      <div key={index} className="space-y-1">
                        ...
                        {phoneErrors[index] && (
                          <p className="text-xs text-red-500">{phoneErrors[index]}</p>
                        )}
                      </div>
                    ))}
                    <Button
                    ...
                    {errors.phone_numbers && (
                      <p className="text-sm text-red-500">{errors.phone_numbers.message}</p>
                    )}
                  </div>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <p className="text-sm text-muted-foreground">
                      {t('createAdvertisement.mediaDescription')}
                    </p>
                    <MediaUpload
              ...
              <Card className="shadow-lg">
                ...
                <CardContent>
                  <FormField 
                    name="status" 
                    label={t('createAdvertisement.status')} 
                    error={errors.status}
                  >
                    <Select 
              ...
              <Card className="shadow-lg">
                <CardContent className="pt-6">
                  <div className="flex justify-end gap-4">
                    <Button
                      type="submit"
                      disabled={createAdvertisementMutation.isPending}
                      className="gradient-primary shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-all hover:scale-105"
                    >
                      {createAdvertisementMutation.isPending ? t('createAdvertisement.creating') : t('createAdvertisement.createAdvertisement')}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
